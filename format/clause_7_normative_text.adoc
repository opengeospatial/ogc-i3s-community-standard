...
The I3S standard supports specifying Coordinate Reference Systems and refers to two OGC standards for describing a CRS as Well Known Text. These are termed `WKT1` as defined in the OGC Coordinate Transformation Service Standard (01-009) and `WKT2` as defined in OGC Geographic Information – Well known text representation of coordinate reference systems (12-063r5). CRS metadata encoded as OGC Well Known Text was originally defined in clause 6.4 in the http://portal.opengeospatial.org/files/?artifact_id=13227[OGC Simple Features 99-036/ISO 19125] Standard. 
...
Please note that in an I3S implementation the CRS MAY be represented using either WKT1 or WKT2. While WKT1 has been in use for many years, WKT1 has been superseded by WKT2. Although WKT2 implementations are not as widely available as WKT1 implementation instances, the guidance from the OGC/ISO community is to implement WKT2. Important Note: WKT1 does not support explicit definition of axis order.
