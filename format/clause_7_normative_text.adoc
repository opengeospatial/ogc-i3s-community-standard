...
The bounding volume hierarchy (BVH) is based on minimum bounding sphere (link:../docs/mbs.cmn.adoc[MBS]) or oriented bounding boxes (link:../docs/obb.cmn.adoc[OBB]). The mesh-pyramids profile supports specifying the bounding volume in either MBS or OBB representation. OBB is the more optimal representation and implementers are encouraged to output node bounding volume in OBB format. Point cloud profile supports OBB representation only.
