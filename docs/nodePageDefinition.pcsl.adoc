...
Nodes represent the spatial index of the data as a bounding volume
hierarchy. To reduce the number of node-index requests required to
traverse this index tree, they are organized in _pages_ of
link:index.pcsl.adoc[layer.index.nodesPerPage] nodes.

Children must be *contiguous*, in index range, so they may be located
using `firstChild` and `childrenCount` fields.
...
The `page_id` of this node is `1`. This is the second page since
...
|*nodes* |link:node.pcsl.adoc[node][] |Array of nodes
|===
