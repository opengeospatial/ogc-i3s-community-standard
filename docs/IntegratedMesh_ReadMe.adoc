...
by an automated process (such as from a drone) for constructing 3D objects out of
...
The following API methods are available for integrated mesh scene layers:
...
|Description |Legacy texture and material description. Not used in OGC Version 1.2 or later. +
`layerID`: Integer. ID of the associated layer. +
`nodeID`: Integer. ID of the associated node.
|===
...
|Description |Description of the node. Not used in OGC Version 1.2 or later. +
`layerID`: Integer. ID of the associated layer. +
