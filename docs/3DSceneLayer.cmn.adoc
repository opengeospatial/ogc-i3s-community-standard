...
store. Every scene layer contains 3DSceneLayerInfo. Features based
...
node never has geometry. Any node’s children represent a higher LoD
quality than an ancestor node. Nodes without geometry at the top of the
tree are allowable since the lowest LoD of a feature/geometry is not
shown.
